...
<html>
<head>
    <title>Search & Book Buses | TripSync</title>
    <style>
        body {
...
<img src="<%= request.getContextPath() %>/images/TripSync.jpg" alt="TripSync Logo" class="logo" width="105px" height="100px">
    ...
    <div class="container">
        ...
        <%
            List<Map<String, String>> buses = (List<Map<String, String>>) request.getAttribute("buses");
            if (buses != null && !buses.isEmpty()) {
                for (Map<String, String> bus : buses) {
        %>
            <div class="card">
                <p><strong><%= bus.get("name") %></strong></p>
                <p>Date: <%= bus.get("date") %></p>
                <p>Price: ₹<%= bus.get("price") %></p>

                <form class="booking-form" action="BusServlet" method="post">
                    <input type="hidden" name="action" value="book">
                    <input type="hidden" name="bus_id" value="<%= bus.get("id") %>">
                    <input type="text" name="passenger_name" placeholder="Name" required>
                    <input type="email" name="email" placeholder="Email" required>
                    <input type="text" name="phone" placeholder="Phone" required>
                    <input type="hidden" name="amount" value="<%= bus.get("price") %>">
        ...
        %>
            <p style="text-align:center; color:#555;">No buses found. Please search above.</p>
        <%
            }
        %>
    </div>
